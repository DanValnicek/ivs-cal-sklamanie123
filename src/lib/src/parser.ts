...
function convertFunctionExpressionsToString(
  inputExpression: string[]
): string[] {
  let indexOfFunction = inputExpression.findIndex(
    (value) => functionTable[value]
  );

  while (indexOfFunction != -1) {
    let functionExpressionIndex =
      inputExpression
        .slice(indexOfFunction)
        .findIndex((value) => value == '(') + indexOfFunction;
    inputExpression.splice(functionExpressionIndex, 1, '$');
    for (
      let nestedParentheses = 0;
      inputExpression[functionExpressionIndex + 1] != ')' ||
      nestedParentheses != 0;

    ) {
      if (inputExpression[functionExpressionIndex + 1] == '(')
        nestedParentheses++;
      if (inputExpression[functionExpressionIndex + 1] == ')')
        nestedParentheses++;

      inputExpression.splice(
        functionExpressionIndex,
        2,
        inputExpression
          .slice(functionExpressionIndex, functionExpressionIndex + 2)
          .join(' ')
      );
    }
    inputExpression.splice(functionExpressionIndex + 1, 1);

    indexOfFunction = inputExpression
      .slice(functionExpressionIndex)
      .findIndex((value) => functionTable[value] != undefined);
  }
...
export function convertRPN(expression: string): Array<number | string> {
  ...
  while (input.length) {
    ...
    if (!isNaN(Number(currentToken))) output.push(Number(currentToken));
    else if (functionTable[currentToken]) operatorStack.push(currentToken);
      ...
      while (
        ...
        (operatorStack.length != 0 &&
          ...
          // @ts-ignore
          operatorTable[operatorStack.at(-1)].precedence >=
            operatorTable[currentToken].precedence &&
      ...
      if (operatorStack.at(-1) != '(')
        throw Error('Left parenthesis not found');
      //remove "("
      operatorStack.pop();
    } else {
  ...
  if (operatorStack.find((element) => element === '(' || element === ')'))
...
export function parseExpression(expression: string): number | undefined {
  ...
  while (termList.length != 1 || typeof termList[0] != 'number') {
    let term = termList.find(
      (value) => operatorTable[value] || functionTable[value]
    );
